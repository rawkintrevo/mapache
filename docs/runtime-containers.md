...
Provisioned sessions also receive `WORKSPACE_SYNC_ROLE` from the Functions lease controller. `writer` owns the workspace's sync-writer lease; `reader` remains available for terminal and inspection work without taking ownership; `none` is used for SSH or sessions that cannot provision. The current runner image still treats this as metadata for the next writer-aware runtime rollout. Functions persists the role on the session and the owner/lease ID on the workspace, releases it on stop/delete/provisioning failure, and periodically reconciles stale owners.
