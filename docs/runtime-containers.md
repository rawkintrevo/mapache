...
Chrome profiles are not part of the visible workspace tree or the general home archive. Legacy/shared runners can still read the compatibility archive at `{workspace.storagePrefix}/.mapache-internal/chrome/chrome-profile.tar.gz`, but the current publisher writes immutable, versioned seeds under `{workspace.storagePrefix}/.mapache-internal/chrome-profile-seeds/v1/` and advances `current.json` only after checksum- and generation-validated publication. `session-runner/lib/chromeProfileSeed.service.js` flushes the filesystem, stages a tar capture, verifies that the source directory stayed stable, and retries boundedly; it never stops or kills an active user browser. The seed descriptor is workspace-owned and includes its schema, immutable version/object generation, checksum, capture timestamp, and compatible browser metadata. The publisher is guarded by the workspace writer authority, and a bounded retention pass preserves current/recent versions plus every version referenced by an automation run.

Private automation runners use `/var/lib/mapache/runtimes/{identity}/chrome/profile` and never publish that profile to shared storage. Functions selects only the last complete seed already published by the same workspace and never requests a live capture during automation startup. Periodic/final publication and the protected explicit snapshot route refresh `current.json` independently. The exact descriptor is pinned on the run/session. `chromeProfileSeed.service.js` verifies ownership, path, object generation, byte length, SHA-256, and schema before restoring into a staging directory; a selected seed failure stops provisioning instead of silently creating an empty browser. A genuine workspace with no seed is reported as `fresh/no_seed`. Restoration runs before `chromeProfile.service.js` and Chromium startup, while periodic/final snapshot calls from automation are no-ops.

The runner stages and sanitizes legacy and inherited profiles before restore, then serializes periodic and final workspace snapshots. Profile replacement first preserves the current directory as a sibling backup; if an image-layer or overlay filesystem rejects either rename with `EXDEV`, it copies into the destination filesystem and removes the source only after the copy completes. An interrupted install removes the partial destination and restores the preserved profile; a failed copy leaves the original profile available for rollback. Profile extraction uses GNU-compatible ownership and permission guards, and reports the tar process error ahead of any secondary stream-close error. Cache, crash, download, lock, socket, and other transient paths are excluded. Shell sessions never create, restore, or overwrite the shared target.
...
prefix, the legacy Chrome profile archive prefix, and the immutable Chrome
profile seed prefix. Publishing or restoring seeds requires the rebuilt
`pi-chrome` image and recreated/restarted runner services; Functions provisioning
must be deployed before enabling the automation canary. No package catalog or
duplicate agent-control API is part of the current deployment.
...
Creation, restart, resize, and recreation derive the bucket and prefix from the
owner-checked workspace record and refresh the session metadata; stale session
storage fields cannot select another workspace's `/automations` prefix.
