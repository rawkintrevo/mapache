...
The `provisionQueuedSession` Firestore trigger watches `workspaces/{workspaceId}/sessions/{sessionId}` writes but only handles records explicitly marked `status: "provisioning"` and `provisioningState: "queued"`. It loads the owning workspace, delegates to the same idempotent Cloud Run provisioning service, and records an actionable failure when worker setup fails. The synchronous create path remains the compatibility path until the session API is switched to write queued records.
