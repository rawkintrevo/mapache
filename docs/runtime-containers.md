...
The backend is authoritative for image selection. `functions/runnerCatalog.helpers.js` and `functions/runnerImages.helpers.js` contain the curated server-side image catalog. Session creation accepts `imageKey`, resolves the catalog entry plus its `harnessId`, persists both on the session document, and then provisions Cloud Run. After create or restart, the backend reads the Cloud Run service template image and records whether it matches the current catalog image for that `imageKey`; stopped, provisioning, and legacy sessions without enough metadata report unknown freshness. Restart re-resolves the current catalog image before patching or recreating Cloud Run so stale sessions pick up new runner image tags. Legacy clients may still submit `image` only when it exactly matches a curated catalog image. Arbitrary user-supplied image URIs are rejected with `invalid_runner_image`.
