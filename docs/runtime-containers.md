...
Pi provider auth is also mirrored in Firestore at `users/{uid}/private/piAuth` under a `providers` map whose entries match Pi's `~/.pi/agent/auth.json` object shape exactly (`providerKey -> credential object`). The backend API writes web-added API keys as `{type: "api_key", key: "..."}`. The runner receives `OWNER_UID`, reads any restored/local `auth.json` entries into that Firestore map, and on startup materializes the merged Firestore providers back into `~/.pi/agent/auth.json` with `0600` permissions. This makes CLI/TUI `/login` additions visible to the web UI after runner sync while preserving the terminal-first auth flow.
