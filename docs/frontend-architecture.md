...
Preview-capable sessions show the Preview tab in `SessionDetail`. Pi sessions expose Models and a Goal button below the canvas; Goal opens the authenticated `WorkspaceGoalsPanel` in the selected-session view and preselects that Pi session for starting or resuming a saved goal. When Pi is already open in Terminal/Chat, a persistent error offers an explicit stop-and-switch action. The Goals panel uses the nested runner runtime state for questions and progress, and adopts the updated goal revision after each answer. Preview sharing and publishing controls are not part of the selected-session toolbar. Sessions whose access response includes `agentUrl` show a capability-gated `Agent` tab rendered by `src/components/sessions/PiWebUiCanvas.jsx`; its iframe is keyed to the selected session and stays mounted while Agent, Chrome, and Preview tabs change. `PiWebUiCanvas` validates the runner origin and child window source for typed postMessage readiness, renewal requests, and status messages, then posts refreshed signed access without changing the iframe `src`. Chrome-capable sessions show a capability-gated `Persistent Chrome` tab rendered by `src/components/sessions/BrowserCanvas.jsx`; its iframe uses the signed `browserUrl` and its toolbar opens the same authenticated browser surface in a new tab. Running Cloud sessions also show `ResourceUtilization` beside the canvas tabs. `src/components/sessions/useResourceMetrics.js` owns the signed `/metrics` WebSocket, reconnects, validates the latest CPU/RAM sample, and keeps no history yet. `ResourceUtilization.jsx` renders accessible green/amber/red meters, while SSH-backed sessions omit the gauges because their runner container does not represent the remote host. Pi sessions with the catalog `chat` capability also show a Chat tab after Terminal, rendered by `src/components/sessions/PiChatCanvas.jsx`; `src/components/sessions/usePiChat.js` owns the session-local WebSocket, replay, reconnect, acknowledgement, and safe-error state, while `PiChatMessage.jsx` renders user text and assistant GFM without raw HTML. Terminal and Chat remain mounted for the selected session while canvas tabs change visibility, preserving their connections, draft, pending-turn, scroll, and iframe state across tab switches. `src/components/sessions/useSessionAccessUrls.js` owns initial signed access loading, renewal before `expiresAt`, and rate-limited renewal after repeated pre-open Chat failures; renewal keeps the current URLs usable if the API request temporarily fails, and the embedded agent bridge rotates the child cookie plus reconnects its existing WebSocket without reloading or replaying prompts. Chat prompts are forwarded to the existing runner PTY, so switching canvases never starts another Pi process or conversation. The Chat canvas keeps a pending user bubble until authoritative transcript replay catches it up and provides an `Open Terminal` fallback for approvals, tools, slash commands, and recovery. `src/components/goals/WorkspaceGoalsPanel.jsx` also renders in the workspace overview when no session is selected. It uses the authenticated API client rather than treating browser state as authoritative, filters start/resume choices to running Pi sessions, polls the assigned runner for pending RPC dialogs, and submits bounded answers. Detailed proposals, task evidence, audit detail, and recovery history require the follow-up work described in `docs/plans/pi-workspace-goals.md`.

The top bar owns workspace selection through a compact dropdown, with adjacent create, edit, and delete actions. The edit action opens `WorkspaceEditModal`, which renames the selected workspace without changing its storage prefix or session state. The left drawer starts with workspace-scoped Files and Sessions sections, exposes session creation only from the Sessions section header for the selected workspace, and gives every session row an edit action. Running session rows show an inline pause action backed by the existing stop lifecycle, and stopped rows replace it with an inline resume action backed by restart. `SessionEditModal` owns session renaming plus the existing resource preset and advanced CPU/memory resize controls; resource sizing is not repeated below the selected terminal. The selected workspace header shows a type tag from `src/components/workspaces/workspaceSourceSummary.js` (`Blank`, `GitHub`, or `Dev machine`) instead of exposing implementation-flavored storage/session prefixes in the primary summary. The left drawer user menu shows an Admin item only when the current profile includes `isAdmin: true`. The Admin page lives in `src/components/admin/AdminPage.jsx` and reads paginated user summaries through `src/services/api.js`; `src/main.js` owns the admin page cursor stack, refresh, and whitelist toggle handlers.
