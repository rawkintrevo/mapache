...
Restart also refreshes source type and sync-policy metadata from the current workspace document before patching or recreating Cloud Run. The workspace is authoritative over stale per-session snapshots, so repairing a workspace to `blank` cannot leave an older session provisioning a GitHub clone or causing the UI to expose Git controls.
...
New workspace creation stamps the server-owned `agentUiVersion: "pi-web-ui-v1"` marker for supported blank and GitHub sources. Session creation ignores client runner selection and always resolves the exact catalog `pi-chrome` image and Pi harness, snapshots the selected workspace's MCP config, validates and normalizes CPU/memory through `functions/sessionResources.helpers.js`, reserves the single workspace runner and sync-writer role in one transaction, and returns before Cloud Run readiness. A second active runner is rejected with `workspace_already_has_active_runner`; same-operation retries converge on the existing session and runtime reservation. The reservation copies `agentRuntimeOperationId`, a positive monotonic `agentRuntimeGeneration`, and `agentRuntimeState` to the new managed session while recording the same admission state on the workspace. Failed/stopped reservations clear the current runtime session ID but retain generation history, so a later admission is deterministic. The `provisionQueuedSession` Firestore trigger and Cloud Run service builder repeat the pi-chrome identity check before any Cloud Run request, so forged image keys, arbitrary image URIs, SSH sessions, and unsupported historical runner records cannot launch. Historical sessions remain listable/readable; restart, resize, and queued provisioning for unsupported records fail closed without converting them. The backend retains curated catalog entries for historical metadata and freshness lookup, but no longer uses `SESSION_RUNNER_IMAGE` or a client image field for ordinary creation. The trigger provisions a per-session Cloud Run service and records service URL/status/image/capability metadata. Active session revisions request one minimum instance, and managed revisions disable CPU idle so background work continues without a browser request. Provisioning treats a Cloud Run `ALREADY_EXISTS` response as an idempotent retry: it polls the fixed per-session service name and adopts the service only after Cloud Run reports it ready. Partial payloads, unknown CPU/memory values, and unsupported resource pairs return HTTP 400 with the stable `invalid_session_resources` error before any Firestore or Cloud Run mutation. Session stop/delete paths clean up Cloud Run services and record allocated runner usage.
The runner then acquires `agentRuntimeBootInstanceId` in a transaction over that same workspace and session, writing `agentRuntimeAuthorityState`, acquisition, and heartbeat timestamps to both records. `functions/syncWriterLease.service.js` clears those boot fields only when the stopping session presents the matching generation and boot ID; a missing or uncertain predecessor is not replaced by a timeout. This distinguishes a physically running Cloud Run container from the one admitted writer, and lets a controlled stop/recreate fence the old boot before a successor generation starts.
...
Historical SSH-backed sessions and Dev machine workspace records remain readable for compatibility and cleanup, but the backend rejects new SSH workspace/session creation and never restarts or provisions an SSH record. The session-scoped SSH routes remain available only for already-running historical services until the later legacy-retirement task removes them.
...
Backend proxy routes verify workspace/session ownership before calling protected runner routes for Git status/actions, workspace skills, workspace subagents, Pi package operations, preview/access URLs, share-preview export, and auth materialization. `createSessionAccessUrls` also returns short-lived signed `browserUrl` and `browserStatusUrl` values for Chrome-capable sessions. Marked QA workspaces may additionally receive an `agentUrl` only when the owned session is running, uses the curated `pi-chrome` image with Pi and Chrome capability metadata, and carries an explicit non-zero runtime generation. Agent URLs use the existing per-session browser secret and TTL but sign `aud: "agent"`, the session ID, and the runtime generation; legacy browser and terminal access fields remain unchanged. The runner serves `/browser/` and `/browser/status` only after validating the HMAC token bound to the session; its `/browser/vnc` WebSocket bridge reaches loopback-only x11vnc, and `/browser/activity` accepts either the browser token or the backend runner token. Raw CDP/VNC ports and the profile archive remain container-internal. Browser terminal/preview access uses finite-lifetime runner URLs signed with the per-session browser secret; backend-only runner management keeps using the shutdown token gate. The deployment-only `scripts/mark-agent-workspace.mjs` helper marks one explicitly identified, owner-verified QA workspace with the fixed `pi-web-ui-v1` marker; browser payloads cannot select the marker, image, or runtime generation.
