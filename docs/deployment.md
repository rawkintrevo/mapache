...
Active session service templates set `minInstanceCount: 1` and `maxInstanceCount: 1`. Marked pi-web-ui services also set the Cloud Run v2 `template.containers[].resources.cpuIdle` field to `false`, keeping CPU available for browser-independent work. Marked services are omitted from the browser-idle reaper; explicit Stop is their lifetime boundary. Unmarked sessions retain the existing idle reaper and template behavior. Marked restart and resize first request the runner's bounded quiesce/final-save acknowledgement, delete the old service, confirm deletion, and only then reserve a new runtime generation and create its replacement. A failed acknowledgement, uncertain deletion, or startup failure leaves safe recoverable metadata and does not start a successor. Deploy Functions before validating this behavior, and restart, resize, or recreate an older session because an existing Cloud Run template retains its previous scaling values. Runner bootstrap-failure reporting additionally requires rebuilt affected standard runner images; older revisions can still leave Firestore at `running` after a later instance-start failure.

The embedded pi-web-ui rollout remains server-owned. A deployment operator can run `node scripts/mark-agent-workspace.mjs --project=pi-agents-cloud --confirm-qa=pi-web-ui-v1 --uid=<owner-uid> --workspace-id=<qa-workspace-id>` with Application Default Credentials to mark exactly one verified QA workspace. The helper writes only the fixed `agentUiVersion` marker and an audit-shaped QA marker; it does not accept image or version input from the browser. Functions forces marked new sessions to the curated `pi-chrome` image and passes the marker plus explicit runtime-generation metadata into the runner. Access responses omit `agentUrl` until all compatibility and generation checks pass. Keep the HubSpot workspace source unmarked.
