...
- The production workflow writes `functions/.env.pi-agents-cloud` with `FUNCTION_SERVICE_ACCOUNT=mapache-api@pi-agents-cloud.iam.gserviceaccount.com` and `SESSION_RUNNER_SERVICE_ACCOUNT=mapache-session-runner@pi-agents-cloud.iam.gserviceaccount.com` before `firebase deploy` because the API function uses Firebase Functions parameters for both its Cloud Functions runtime identity and the runtime identity assigned to per-session Cloud Run services. Non-interactive CI deploys fail if either value is absent from Firebase's dotenv-resolved parameter files.
