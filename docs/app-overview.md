...
- The production workflow writes `functions/.env.pi-agents-cloud` with `FUNCTION_SERVICE_ACCOUNT=mapache-api@pi-agents-cloud.iam.gserviceaccount.com` before `firebase deploy` because the API function uses that Firebase Functions parameter as its Cloud Functions runtime identity. Non-interactive CI deploys fail if this value is absent from Firebase's dotenv-resolved parameter files.
