...
For connected GitHub workspaces, Pi sessions also have an automatic branch/PR lifecycle. During runner startup, after repository/cache restore and before Pi starts, the runner uses the GitHub App installation token to reset to the selected base branch and create a unique `mapache/<session-name-kebab>-<session-id>` branch. This prevents the agent from working directly on the source branch. When the Pi terminal exits, the runner stages all changes, commits them, pushes the branch, and opens a pull request. If the session exits without file changes, no commit or PR is created. Shell sessions remain manual so users can inspect or intervene without triggering automatic PR creation.
