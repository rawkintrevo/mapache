...
GitHub-backed live sessions expose repository actions in the navigation drawer below Sessions. The section shows the current branch and pull/push controls. Its Git manager modal shows the current commit, ahead/behind state, and separate staged, unstaged, and untracked file groups. Users can stage/unstage files, add an exact untracked path to the root `.gitignore`, commit staged changes, and open a pull request. The modal lists local and fetched remote branches, checks out an existing local branch or creates a tracking branch for a remote-only branch, and can create a new branch from the current commit. Branch switches use ordinary Git checkout behavior: uncommitted changes are carried when Git permits it, and switches that would overwrite changes fail without an automatic stash or discard. The controls are hidden for blank, SSH, stopped, or otherwise non-live sessions. The manual `Push` action only sends the current branch; it does not stage or commit uncommitted edits first. Connected GitHub App repositories push through short-lived installation-scoped credentials minted by the backend and passed to the runner for that single action; public URL workspaces still need runner-provided push credentials. Pull request creation is limited to connected GitHub App repositories for the same reason.
