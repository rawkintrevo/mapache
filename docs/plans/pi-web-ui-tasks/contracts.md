...
- Task 13 adds `agentRuntimeBootInstanceId`, `agentRuntimeAuthorityState`, `agentRuntimeBootAcquiredAt`, and `agentRuntimeBootHeartbeatAt` to both the reserved workspace and session. The runner atomically acquires a unique boot ID only when the session and generation still match; a non-empty different boot ID is never taken over. Controlled stop/recreate clears the old boot ID before reserving the next generation. `workspaceAuthority.js` renews the admission with bounded Firestore transactions, and `assertCurrentWriter` checks both documents before runtime commands, file sync, or later checkpoint publication. A failed or indeterminate coordination read revokes local admission, rejects new work, and terminates the managed process group. Physical Cloud Run container presence is not writer admission: a running container without the current boot ID must remain fenced, and a missing heartbeat alone cannot authorize replacement.
