...
The backend starts authorization with a signed, short-lived, single-use state containing the user, workspace, nonce, reconnect intent, and requested services. Before exchanging the code, the callback atomically creates a nonce record at `users/{uid}/private/googleOAuthState/attempts/{nonce}` in Firestore. This makes replay protection durable across Functions instances and cold starts; a second callback is rejected before token exchange. These records contain no tokens and retain their signed expiry for cleanup. Initial authorization uses account selection, while an explicit reconnect/change-account flow requests consent and does not fall back to an old refresh token if Google omits a replacement.
