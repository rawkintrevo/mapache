...
- Pi/package/workspace-skill endpoints: `session-runner/lib/pi.js`, `piPackage.service.js`, `piSkill.service.js`
...
The terminal uses `node-pty` and WebSocket replay. The terminal iframe HTML in `terminal.js` also inlines the critical xterm layout rules that hide the helper textarea and anchor the viewport/screen, then reapplies hidden helper-textarea styles and clears stale helper input after key/input/render events. That runner-side hardening prevents transient helper-input characters from remaining visible when xterm temporarily repositions its hidden textarea over the cursor cell. Preview routes support static, proxy, and N64 ROM modes depending on runner capabilities and workspace preview config. Web-capable images also expose a runner-owned browser QA contract: `browserQa.js` reports dependency health into `/capabilities`, `/preview/status`, and `/preview/qa/status`, while the image-local `mapache-preview-qa` command launches Chromium through Playwright, writes structured reports under `$MAPACHE_QA_DIR`, and updates a shared `last-run.json` state file that status routes can surface. GitHub workspaces restore `.git` through archives or clone fallback, then restore worktree/cache state. Pi package and skill endpoints operate on the same `/workspace/.pi` files that Pi uses in the terminal.
...
Workspace skill CRUD now uses neutral runner routes at `/skills` and `/skills/delete`. `piSkill.service.js` resolves the active harness from `config.terminalKind`, returns harness metadata and restart guidance with list/save/delete results, and keeps Pi legacy flat-file deletion support for historical `.pi/skills/{name}.md` entries. `server.js` still serves `/pi/skills*` aliases for rollout compatibility.
